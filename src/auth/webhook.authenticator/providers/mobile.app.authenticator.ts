...
export class MobileAppAuthenticator implements IWebhookAuthenticator{
    ...
    get tokens(): WebhookAuthTokens {
        const urlToken = this._envService.getTenantEnvironmentVariable('WEBHOOK_REANAPP_CLIENT_URL_TOKEN');
        const headerToken = this._envService.getTenantEnvironmentVariable('WEBHOOK_REANAPP_CLIENT_HEADER_TOKEN');
    ...
    authenticate(request: express.Request): void {
        ...
        throw new Error(`Unable to authenticate webhook request from mobile-app for tenant ${request.tenantName}.`);
    }
